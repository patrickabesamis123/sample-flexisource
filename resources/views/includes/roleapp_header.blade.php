<header class="header pvm-header pvm-header--2  clear-float ng-scope pvm-candidate">
        <section class="pvm-logo-handler">
        <a href="/" class="brand" rel="home" ng-click="goToLink($event)"><img src="{{ asset('images/logo.png') }}" title="PreviewMe"></a>
        </section>
        <section class="pvm-search-handler pvm-phone-invisible">
          <input type="text" class="pvm-input-text pvm-search ng-pristine ng-untouched ng-valid ng-empty" id="search_top" placeholder="Search for jobs, companies, and more..." ng-model="globalsearch" ng-enter="search()">
          <i class="fa fa-search" ng-click="search()"></i>
        </section>
        <section class="pvm-nav-handler">
          <a href="/settings#" ng-click="navigateUrl('/employer/add/job')" class="btn-pvm btn-primary ng-hide" ng-show="is_employer" style="float:left; width: auto;">Create Roles</a>
          ...
          <a href="/my-profile" data-ajax="false" title="Profile" class="head__profile-link">
            <div class="randomInitialColor member-initials member-initials--nav replace">　</div>
            <img class="img-circle pvm-profile-thumb" src="{{ asset('images/human-o.png') }}">
          </a>
          ...
          <span class="setttingsLink head__settings-link" data-ajax="false" title="Messages" ng-init="headOpenSet=false">
            ...
            <ul class="head__setting-list ng-hide" ng-show="headOpenSet">
