(function () {
  ...
  function checkBrowsers() {
	...
	// remove duplicates from an object IB7
    app.filter('uniquemembers', function() {
	...
	app.controller('EmployerJobCreate', ['GlobalConstant','RequirementsToPreApply','fileUploadService',   '$scope','$window','$http','$cookies','$filter','$location', '$anchorScroll', 'JobToTemplate', '$timeout', function(GlobalConstant,RequirementsToPreApply, fileUploadService, $scope, $window, $http, $cookies,$filter,$location, $anchorScroll, JobToTemplate, $timeout) {
		...
		if (angular.isDefined($scope.token)) {
			...
			var i  = 0;
			$scope.JobAvailabitily = [];

			while(i < 31) {
			 	i++
			 	text = i == 1 ? i + " day" : i + " days";
			 	var val = i ;
			 	$scope.JobAvailabitily.push({text: text , value: val });
			}

			$scope.JobAvailabitily.push(
			 	{ text: "45 days", value: 45},
			 	{ text: "60 days", value: 60},
			 	{ text: "90 days", value: 90}
			)

			// console.log($scope.JobAvailabitily)
			...
			$scope.populateLocation = function(jobdata) {
				...
				.then(function(response) {
					...
					//Check if there is result from autocomplete list
	      ...
	      if (angular.element($('#autoDataLocation')).is(':visible')) {
			...
			// console.log("comp ben: ", $scope.benefits);
			...
			if (angular.isDefined($scope.URLQS.id) && $scope.URLQS.id   != '' &&  $scope.URLQS.id != true) {
				...
				$.ajax({
					...
					success : function(response) {
						...
						$scope.preload = true;
						// console.log("jobdata1: ", $scope.JobData);
					}
				});
			}
			//Populate field if template is loaded
				...
				$.ajax({
					...
					success : function(response) {
						...
						$scope.preload = true;
						// console.log('template loaded', $scope.jobData);
					}
				});
	    	...
	    	//Populate Role duration
	    	if (angular.isDefined($scope.JobData.role_duration)) {
		    	var selected_role_duration = $filter('filter')($scope.JobAvailabitily, {value: $scope.JobData.role_duration}, true);
					angular.forEach(selected_role_duration, function(val, key) {
						var index = $scope.JobAvailabitily.indexOf(val);
						$scope.job_availability = $scope.JobAvailabitily[index];
					});
				}
	  	...
	  	$http.get(GlobalConstant.StaticOptionsApi + '/work_type').then(function(response) {
			...
			//Field Control
			$scope.validatethis = true;
			$scope.publishing = false;
			$scope.EmployeeAddJobStep1 = function (isdraft) {

        if ((($('#location')[0].checkValidity() == false || $('#roleCountry')[0].checkValidity() == false) && browserName == "Netscape" && majorVersion == 5) || (($('#location')[0].checkValidity() == false || $('#roleCountry')[0].checkValidity() == false) && browserName == "Safari" && majorVersion == 6)) {
          checkBrowsers();
          alert('Country/Location is a required field.');
        } else {
          if ((isdraft == 'draft' && $('#formStep1')[0].checkValidity() == false) || (isdraft == 'draft' && $('#location')[0].checkValidity() == false)) {
            //console.log('check fields')
            alert('Cant save just yet! please check all the required fields before proceeding.');

            $scope.validatethis = false;
          ...
          if ($scope.LocationId != null) {
            ...
            $scope.locationName = $scope.searchLocation;
          } else {
            ////console.log('2')
            if ($scope.searchLocation != '') {
              ...
              $scope.locationName = $scope.country.display_name;
            }
          }
          ...
          $scope.AllData = {
            "data": {
              ...
              "role_duration": parseInt($scope.job_availability.value),
          ...
          if (angular.isUndefined($scope.URLQS.id) || $scope.URLQS.id == '' || $scope.URLQS.id == true) {
            ...
            .then(function (response) {
              // console.log("EmployerAddJob_True: ", response.data.data);
              ...
              if (angular.isDefined($scope.JobData)) {
                if (angular.isDefined($scope.JobData.location)) {
                  var location = $scope.JobData.location.data.display_name;
                } else {
            ...
            .then(function (response) {
              ...
              //console.log("Put methods: ", GlobalConstant.EmployerAddJob, $scope.URLQS.id, $scope.AllData);
	...
	app.controller('EmployerJobCreateStep2', ['GlobalConstant','RequirementsToPreApply','fileUploadService', 'ReloadJobDetailsOnSlideSwitch','$scope','$window','$http','$cookies','$filter', '$timeout', '$compile','$location', function(GlobalConstant,RequirementsToPreApply,fileUploadService,ReloadJobDetailsOnSlideSwitch, $scope, $window, $http, $cookies,$filter, $timeout, $compile,$location) {
	    ...
	    $('#carousel-example-generic').on('slid.bs.carousel', function() {
				...
				if($('#TEstMe section.active').hasClass('formstep2')) {
					...
					if (angular.isDefined($scope.URLQS.id) && $scope.URLQS.id != '' &&  $scope.URLQS.id != true) {
		    			...
		    			success : function(response) {
		    				$scope.JobData = response.data;
		    				$scope.LoadStep2Data = false;
			    		}
			    	});

						//Populate Pre-apply from step 1
						if ($scope.JobData.questions.pre_apply.length == 0 && angular.isUndefined($cookies.get('loadTemplate'))) {
	    				angular.forEach($scope.JobData.requirements, function(val, key) {
						...
						//override $scope.JobData if has template id and question are empty
						if ($scope.JobData.questions.pre_apply.length == 0 && $scope.JobData.questions.application.length == 0 && angular.isDefined($cookies.get('loadTemplate'))) {
							$scope.JobIsActive = false;
	  		 			$.ajax({
			    			url : GlobalConstant.EmployerRootApi + '/company/template/job/'+$cookies.get('loadTemplate'),
			    			method : 'get',
	    			...
	    			if ($scope.JobData.application_requirements.icebreaker_video != null || angular.isUndefined($scope.JobData.application_requirements.icebreaker_video) == false) {
							...
							$scope.RequirementsList.icebreaker_video  = $scope.JobData.application_requirements.icebreaker_video;
						} else {
							$scope.req_1 = 'no';
						}
						...
						if ($scope.JobData.application_requirements.portfolio != null || angular.isUndefined($scope.JobData.application_requirements.portfolio) == false) {
							...
							$scope.RequirementsList.portfolio  = $scope.JobData.application_requirements.portfolio;
						} else {
							$scope.req_2 = 'no';
						}
						...
						if ($scope.JobData.application_requirements.about_me != null || angular.isUndefined($scope.JobData.application_requirements.about_me) == false) {
							...
							$scope.RequirementsList.about_me  = $scope.JobData.application_requirements.about_me;
						} else {
							$scope.req_3 = 'no';
						}
						...
						if ($scope.JobData.application_requirements.references != null || angular.isUndefined($scope.JobData.application_requirements.references) == false) {
							...
							$scope.RequirementsList.references  = $scope.JobData.application_requirements.references;
						} else {
							$scope.req_4 = 'no';
						}
						...
						if ($scope.JobData.application_requirements.education != null || angular.isUndefined($scope.JobData.application_requirements.education) == false) {
							...
							$scope.RequirementsList.education  = $scope.JobData.application_requirements.education;
						} else {
							$scope.req_5 = 'no';
						}
						...
						if ($scope.JobData.application_requirements.work_experience != null || angular.isUndefined($scope.JobData.application_requirements.work_experience) == false) {
							...
							$scope.RequirementsList.work_experience  = $scope.JobData.application_requirements.work_experience;
						} else {
							$scope.req_6 = 'no';
						}
						...
						if ($scope.JobData.application_requirements.resume != null || angular.isUndefined($scope.JobData.application_requirements.resume) == false) {
							...
							$scope.RequirementsList.resume  = $scope.JobData.application_requirements.resume;
						} else {
							$scope.req_7 = 'no';
						}
	    	...
	    	fileUploadService.video_upload($scope, file_elm, evt, 'employer_jobs', getJobId);
			}
      ...
      function checkBrowsers() {
	    ...
	    $scope.saveVideo = function() {
	      ...
	      fileUploadService.saveVideo($scope, 'employer_jobs', getJobId);
	      // $scope.videoCreateRole = true;
			...
			$scope.EmployeeAddJobStep2 = function (isdraft) {
				...
				if ($scope.PreArrovalQuestions.length != 0) {
					angular.forEach($scope.PreArrovalQuestions , function (value, key) {
						if (angular.isDefined(value.ideal_answer) && value.ideal_answer.length != 0) {
							delete value.id;
							delete value.$$hashKey;
							...
							var Thisposition = angular.element('#StartPreApproval').offset();
							angular.element($("html, body")).animate({ scrollTop: Thisposition.top }, 1000);
							$('.err_msg'+key).show();
							return false;
						}
					});
				...
				//validate pre apply question before submitting
				if ($scope.PreArrovalQuestions.length != $scope.dataPreApproval.length) {
					return false;
				}
			    ...
			    $scope.preApply.then(function(response) {
					...
					if (angular.isDefined(getQueryString.draft) && getQueryString.draft == 1) {
						var sq_method = "PUT";
						$scope.StandardQuestionMethod = $http.put(StandardQuestionURL, StandaraQuestionAllData);
					} else {
						if (StepMethodStandardQuestion != 1) {
							var sq_method = "post";
							$scope.StandardQuestionMethod = $http.post(StandardQuestionURL, StandaraQuestionAllData);
							$cookies.put('step2', 1, { 'path':'/'});
		    		...
		    		if (angular.isDefined(isdraft) && isdraft == 'draft') {
		    			 ...
		    			 alert('The current role has been saved as a draft. You will find it on the Company Roles tab of your Dashboard');
		    			 $location.search('id', $scope.JobData.id);
		    			 $location.search('draft', 1);
		    			 //$window.location.href = base_url+'employer/jobs';
		    		} else {
		    			$('.step3').trigger('click');
	...
	app.controller('EmployerJobCreateStep3', ['GlobalConstant','$scope','$window','$http','$cookies','$filter', '$timeout', '$compile','$location', '$modal', '$log', 'JobToTemplate',
	function(GlobalConstant, $scope, $window, $http, $cookies,$filter, $timeout, $compile,$location, $modal, $log, JobToTemplate) {
		...
		angular.element('#carousel-example-generic').on('slid.bs.carousel', function() {
			$scope.getJobIdonSlide = $scope.URLQS.id;
			$scope.getTemplateonSlide = $cookies.get('loadTemplate');
			var tempname = JobToTemplate.ToTemplate();

			if (tempname.length != 0) {
				$scope.savetemplate.name = tempname[0];
			}
			$scope.disableslide = false;
		...
		if (angular.isDefined(getJobId) && angular.isDefined($cookies.get('loadTemplate')) ||  angular.isUndefined(getJobId)     && angular.isDefined($cookies.get('loadTemplate'))) {
	  	...
	  	$scope.JobIsActive = false;
			...
			if (angular.isDefined($scope.JobData)) {
			  ...
			  ////console.log(data)
	  		// var gettemplatename = $scope.JobData.job_title +' - '+ $scope.JobData.location.data.display_name + ' - '+data;
	  		// if (angular.isArray(gettemplatename)) {
	  		// 	var settemplateName = gettemplatename[0]
	  		// } else {
	  		// 	var settemplateName = gettemplatename
	  		// }
	  		// $scope.savetemplate.name = settemplateName;
      ...
      var b =1;
      for (var a=0; a<$scope.GetAllCompanyTeam.length; a++) {
        for (var i = 0; i < $scope.GetAllCompanyTeam[a].members.length; i++) {
          if (b >= 6) {
            b = 1;
          }

          $scope.F_initial = $scope.GetAllCompanyTeam[a].members[i].employer.first_name;
          $scope.F_initial = $scope.F_initial.substr(0, 1);

          $scope.L_initial = $scope.GetAllCompanyTeam[a].members[i].employer.last_name;
          $scope.L_initial = $scope.L_initial.substr(0, 1);

          $scope.GetAllCompanyTeam[a].members[i].employer.initial = $scope.F_initial + $scope.L_initial;

          // change default photo's background color

          if (!$scope.GetAllCompanyTeam[a].members[i].employer.profile_picture_url) {

            if (b == 1) {
              $scope.GetAllCompanyTeam[a].members[i].employer.profile_color = "member-initials--sky";
            }
            else if (b == 2) {
              $scope.GetAllCompanyTeam[a].members[i].employer.profile_color = "member-initials--pvm-purple";
            }
            else if (b == 3) {
              $scope.GetAllCompanyTeam[a].members[i].employer.profile_color = "member-initials--pvm-green";
            }
            else if (b == 4) {
              $scope.GetAllCompanyTeam[a].members[i].employer.profile_color = "member-initials--pvm-red";
            }
            else if (b == 5) {
              $scope.GetAllCompanyTeam[a].members[i].employer.profile_color = "member-initials--pvm-yellow";
            }
            b++;
          }
        }
        ...
        // team default icon
		    $scope.team_initial = $scope.GetAllCompanyTeam[a].team_name;
		    $scope.team_initial = $scope.team_initial.replace(/[^A-Z]/g, "");
		    $scope.GetAllCompanyTeam[a].initial = $scope.team_initial
		    var color_bg_initial = color_bg_initial_set[Math.floor(Math.random()*color_bg_initial_set.length)];
        $scope.GetAllCompanyTeam[a].team_color = color_bg_initial;
      }



      //console.log("GetAllCompanyTeam: ", $scope.GetAllCompanyTeam[0]);
		...
		//Get All member List //IB
		//console.log("API get members: ", GlobalConstant.APIRoot+'employer/company/member')
		...
		.then(function(response) {
            ...
            // console.log("Get all members: ", $scope.GetAllCompanyMember); // IB
            ...
            if ($scope.JobData) {
                ...
                //console.log(angular.isDefined( $scope.JobData.job_manager))
                if (angular.isDefined( $scope.JobData.job_manager) && $scope.JobData.job_manager != null) {
                    if (angular.isDefined($scope.JobData.job_manager.id)) {
                        $scope.SelectedJobManager.id = $scope.JobData.job_manager.id;
                    } else {
            ...
            // angular.forEach($scope.selectedData.data.member, funtion(value, key){
            // 	$scope.selectedmembers.push('id :' + value);
            // 	console.log("selectedmembers:", $scope.selectedmembers);
            // });

			// console.log("test3: ", $scope.selectedData.data.member);
		  	// Add initial to be used in default image
			var b =1;
        ...
        // console.log("Teamz: ", $scope.selectedData.data.team);
	  ...
	  $scope.ShowHidemanager = function () {
        ...
        //console.log($scope.GetAllCompanyMember);
        // console.log("Teyst: ", $scope.selectedData.data.member);
	    }, function () {
		...
		////console.log($scope.JobData)
		if ($scope.JobData) {
			//if(angular.isDefined($scope.JobData)  && angular.isDefined(getJobId)  && angular.isUndefined($cookies.get('loadTemplate'))  && angular.isDefined($scope.URLQS.draft) && $scope.URLQS.draft == '1' && $scope.JobData.workflow_steps.length != 0)
			if(angular.isDefined(getJobId) && angular.isUndefined($cookies.get('loadTemplate')) && $scope.JobData.workflow_steps.length != 0) {
	  ...
	  $scope.$watchCollection('selectedflow.id', function(newVal, oldVal) {
		...
		$scope.EmployeeAddJobStep3 = function (isdraft) {
		  //PLFE-140 start
		  // console.log("job stat: ", $scope.JobData);
      $.ajax({
        url : GlobalConstant.EmployerAddJob+'/'+$scope.URLQS.id,
        method : 'get',
        headers : {Authorization: "Bearer "+$cookies.get('token')},
      ...
      //PLFE-140 end
			...
			if (getJobId != null) {
		    	...
		    	.then(function(response) {
		    		if ($scope.JobData.job_status == "active" || $scope.JobData.job_status == "draft") { // ZAP-28
	...
	function (GlobalConstant, $scope, $cookies, $http, $modalInstance, $location, data) {
		...
		$scope.ok = function () {
            ...
            //PLFE-105 end
			   		...
			   		// reassignData()
		   			// console.log("not invited: ", response.data.errors);
